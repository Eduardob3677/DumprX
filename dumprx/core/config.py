...
class Config:
    def __init__(self):
        ...
        self.partitions = [
            ...
            "recovery", "tz", "oppo_product", "preload_common", "opproduct", "reserve", "india", 
            "my_preload", "my_odm", "my_stock", "my_operator", "my_country", "my_product", 
            "my_company", "my_engineering", "my_heytap", "my_custom", "my_manifest", "my_carrier", 
            "my_region", "my_bigball", "my_version", "special_preload", "system_dlkm", 
            "vendor_dlkm", "odm_dlkm", "init_boot", "vendor_kernel_boot", "odmko", "socko", 
            "nt_log", "mi_ext", "hw_product", "product_h", "preas", "preavs"
        ]
        
        self.ext4_partitions = [
            "system", "vendor", "cust", "odm", "oem", "factory", "product", "xrom", "systemex", 
            "oppo_product", "preload_common", "hw_product", "product_h", "preas", "preavs"
        ]
        
        self.other_partitions = [
            "tz.mbn:tz", "tz.img:tz", "modem.img:modem", "NON-HLOS:modem", 
            "boot-verified.img:boot", "recovery-verified.img:recovery", "dtbo-verified.img:dtbo"
        ]
    ...
    def _setup_tool_paths(self):
        self.tool_paths = {
            ...
            'packspareimg': self.utils_dir / "bin" / "packsparseimg",
        ...
        self.bin_7zz = self._find_7zz()
    
    def _find_7zz(self):
        if os.system("command -v 7zz > /dev/null 2>&1") == 0:
            return "7zz"
        else:
            return str(self.utils_dir / "bin" / "7zz")
    
    @property
    def github_token_file(self):
        return self.project_dir / ".github_token"
    
    @property 
    def gitlab_token_file(self):
        return self.project_dir / ".gitlab_token"
    
    @property
    def tg_token_file(self):
        return self.project_dir / ".tg_token"
    
    @property
    def tg_chat_file(self):
        return self.project_dir / ".tg_chat"